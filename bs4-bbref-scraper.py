...
class scrape_baseball_reference():
  ...
  def __init__(self):
    print("Pass a dictionary of League year id's to get_league(dictoinary_of_league_year_ids = ___ )")
  ...
  def league_year_player_data(self, league_year_id):
  ...
  def get_league_history(self, dictionary_of_year_ids):
    print("Parsing League History from Baseball Reference...")
    return [self.league_year_player_data(year) for year in dictionary_of_year_ids.values()]
  ...
  # Cleaning
  def concat(list_of_parsed_league_data):
    mooshed_data = [pd.concat(year) for year in list_of_parsed_league_data[1:]]
    return mooshed_data
...
Northwoods_league_Data = Northwoods_league.get_league_history(nwl_yearid_dict)